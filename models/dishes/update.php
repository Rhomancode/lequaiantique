...
if(isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $detail = $pdo->prepare("SELECT * FROM dishes WHERE id = :id");
    $detail->bindValue(':id', $id);
    if($detail->execute()) { 
        $disheDetail = $detail->fetch();
        ...
        if(!empty($_POST)) {
            ...
            && !empty($_POST['description']) && !empty($_POST['price'])) {
                ...
                require_once('../core/config.php');
                $update = $pdo->prepare('UPDATE dishes SET name = :name, description = :description, price = :price WHERE id = :id');
